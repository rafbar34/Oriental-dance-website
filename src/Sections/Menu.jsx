...
export const Menu = ({ myRefHome, myRefAboutsUs, myRefGallery, myRefOffer, myRefVideo, myRefContact }) => {
    ...
    return (
        <div className='bg-black font-link-2  z-30  w-full min-h-24 h-28 fixed text-white flex justify-center'>
            ...
            <div className='flex justify-evenly   w-full text-sm sm:text-xl items-center text-center cursor-pointer'>
                {menuValues.map((menuValue) => (
                    <div onClick={() => { executeScroll(menuValue.ref) }} className='w-1/8 mx-2 hover:opacity-75 sm:text-3xl'>{menuValue.title}</div>
                ))}
            </div>
        </div>
    )
